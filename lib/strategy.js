...
function OAuth2Strategy(options, verify) {
  ...
  if (!options.getClient && !options.clientID) {
    throw new TypeError('OAuth2Strategy requires a clientID option or getClient function callback');
  }
...
OAuth2Strategy.prototype.initAuth2 = async function(req) {
  var clientId, clientSecret;

  if (this._initOptions.getClient) {
    var client = await this._initOptions.getClient(req);
    clientId = client.id;
    clientSecret = client.secret;
  } else {
    clientId = this._initOptions.clientID;
    clientSecret = this._initOptions.clientSecret;
  }
  ...
  this._oauth2 = new OAuth2(
    ...
    clientSecret,
    '',
    this._initOptions.authorizationURL,
    this._initOptions.tokenURL,
    this._initOptions.customHeaders
  );
};
...
OAuth2Strategy.prototype.authenticate = async function(req, options) {
  options = options || {};
  await this.initAuth2(req, options);
